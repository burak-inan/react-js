...
const TodoApp = () => {
  ...
  const createNote= (title,dateTime) => {
    setFormData(formData.concat({ title:title.current.value, dateTime:dateTime.current.value, id:lastId.toString() }))
  ...
  useEffect( ()=> {
    ...
    fetch("https://650e14cfa8b42265ec2cef65.mockapi.io/todo/notes")
    .then(note=> note.json())
  ...
  return (
    <Card className='m-5 p-2 border-info'>
      <Row className='g-4'>
        ...
        <Col sm={9}><Notes {...{formData}} updateNotes={updateNotes}/></Col>
      </Row>
    </Card>
  )
}
